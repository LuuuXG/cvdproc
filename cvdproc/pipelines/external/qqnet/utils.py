...
def save_nii(data, filename, filename_sample=''):
    ...
    import numpy as np
    if filename_sample:
        ref = nib.load(filename_sample)
        img = nib.Nifti1Image(data, ref.affine, ref.header)
    else:
        img = nib.Nifti1Image(data, np.eye(4))  # avoid None affine
...
def extract_patches(img, patch_shape, extraction_step):
    """
    N-D patch extraction using skimage.util.view_as_windows.
    Args:
        img: np.ndarray, shape (D1, D2, D3[, ...])
        patch_shape: iterable of ints, patch size per spatial dim, e.g. (p1, p2, p3)
        extraction_step: iterable of ints, stride per spatial dim, e.g. (s1, s2, s3)
    Returns:
        patches: np.ndarray, shape (N, *patch_shape)
                 where N = prod_i floor((Di - pi)/si + 1)
    """
    patch_shape = tuple(int(x) for x in patch_shape)
    extraction_step = tuple(int(x) for x in extraction_step)

    if img.ndim != len(patch_shape):
        raise ValueError(f"img.ndim ({img.ndim}) must match len(patch_shape) ({len(patch_shape)}).")

    if len(extraction_step) != len(patch_shape):
        raise ValueError(f"len(extraction_step) must equal len(patch_shape). Got {len(extraction_step)} vs {len(patch_shape)}")

    # windows shape: (n1, n2, n3, ..., p1, p2, p3, ...)
    windows = view_as_windows(img, window_shape=patch_shape, step=extraction_step)

    # reshape to (N, *patch_shape)
    num_patches = np.prod(windows.shape[:img.ndim], dtype=int)
    patches = windows.reshape((num_patches,) + patch_shape)
