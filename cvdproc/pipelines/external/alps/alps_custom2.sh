...
-b BVAL \n\
-c BVEC \n\
-p T1w to MNI warp (if available) \n\
...
while getopts 'a:b:c:p:m:i:j:k:n:d:e:r:t:v:h:w:f:s:o:' flag; do
  case "${flag}" in
	...
	p) t1w_to_mni_warp="$(echo "$(cd "$(dirname "${OPTARG}")" && pwd)/$(basename "${OPTARG}")")" ;;
...
then
	...
	if [ "$template" != "0" ]; then #analysis in template space
		if [ -f "$struct" ]; then #if you have structural MRI data
			...
			echo "Registration to template via structural scan using mri_synthmorph";
			...
			#flirt -ref "${outdir}/${smri}_brain.nii.gz" -in "${outdir}/dti_FA.nii.gz" -dof 6 -omat "${outdir}/dti2struct.mat"
   			if [ $weight == "1" ]; then
				mri_synthstrip -i "${outdir}/${smri}.nii.gz" -o "${outdir}/${smri}_brain.nii.gz"

				flirt -ref "${outdir}/${smri}_brain.nii.gz" -in "${outdir}/dti_FA.nii.gz" -dof 6 -out "${outdir}/dti_FA_2_${smri}.nii.gz" -omat "${outdir}/dti2struct.mat"
			...
			# if the warp already exists from a previous run, copy it ($t1w_to_mni_warp)
			if [ -f "$t1w_to_mni_warp" ]; then 
				echo "Warp from structural to template already exist, copying them to the output folder"
				cp "$t1w_to_mni_warp" "${outdir}/struct2template_warp.nii.gz"
			else
				echo "Warp and/or affine from structural to template do not exist, creating them with mri_synthmorph"
				...
				mri_synthmorph -t "${outdir}/struct2template_warp.nii.gz" "${outdir}/${smri}.nii.gz" "${template}" -g
			fi
			mri_convert -at "${outdir}/struct2template_warp.nii.gz" "${outdir}/dti_FA_2_${smri}.nii.gz" "${outdir}/dti_FA_to_${template_abbreviation}.nii.gz"
			
			flirt -in "${outdir}/dti_MD.nii.gz" -ref "${outdir}/${smri}_brain.nii.gz" -out "${outdir}/dti_MD_2_${smri}.nii.gz" -init "${outdir}/dti2struct.mat" -applyxfm
			flirt -in "${outdir}/dti_tensor.nii.gz" -ref "${outdir}/${smri}_brain.nii.gz" -out "${outdir}/dti_tensor_2_${smri}.nii.gz" -init "${outdir}/dti2struct.mat" -applyxfm

			mri_convert -at "${outdir}/struct2template_warp.nii.gz" "${outdir}/dti_MD_2_${smri}.nii.gz" "${outdir}/dti_MD_to_${template_abbreviation}.nii.gz"
			mri_convert -at "${outdir}/struct2template_warp.nii.gz" "${outdir}/dti_tensor_2_${smri}.nii.gz" "${outdir}/dti_tensor_in_${template_abbreviation}.nii.gz"
			...
			if [ "$warp" == "0" ]; then echo "Registration to template with mri_synthmorph";
			mri_synthmorph -o "${outdir}/dti_FA_to_${template_abbreviation}.nii.gz" -t "${outdir}/dti2template_warp.nii.gz" "${outdir}/dti_FA.nii.gz" "${template}" -g
			mri_convert -at "${outdir}/dti2template_warp.nii.gz" "${outdir}/dti_MD.nii.gz" "${outdir}/dti_MD_to_${template_abbreviation}.nii.gz"
   			mri_convert -at "${outdir}/dti2template_warp.nii.gz" "${outdir}/dti_tensor.nii.gz" "${outdir}/dti_tensor_in_${template_abbreviation}.nii.gz"
...
# if warp file already exists, delete it
if [ -f "$t1w_to_mni_warp" ]; then 
	echo "Warp from structural to template already exist, deleting the copy in the output folder"
	rm "${outdir}/struct2template_warp.nii.gz"
fi
