...
%% Custom Settings
cvdproc_directory = 'E:/Codes/cvdproc';

%% Necessary preparation

% Set x-separation tool directory path
home_directory = fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'Chisep_Toolbox_v1.2');
...
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'xiangruili-dicm2nii-3fe1a27')))
...
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'nifti_toolbox')))
...
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'STISuite_V3.0')))

% MEDI toolbox (http://pre.weill.cornell.edu/mri/pages/qsm.html)
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'MEDI_toolbox')))
...
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'data', 'matlab_toolbox', 'mritools_Windows_3.5.5')))
...
RunOptions.OutputPath = 'F:/BIDS/7T_SVD/derivatives/sepia_qsm/sub-SVD7TNEW01/ses-baseline';
...
elseif strcmp(RunOptions.multi, 'single')
    single_subj_path = 'F:/BIDS/7T_SVD/derivatives/sepia_qsm/sub-SVD7TNEW01/ses-baseline';
...
elseif strcmp(RunOptions.InputType, 'nifti')
    pathNifti_mag = [fullfile(subj_dir(subj).folder,subj_dir(subj).name),'/Sepia_part-mag.nii'];
    pathNifti_phs = [fullfile(subj_dir(subj).folder,subj_dir(subj).name),'/Sepia_part-phase.nii'];
    % magnitude
    ...
    Data.B0_strength = 7;
    Data.B0dir = [0, 0, 1];
    Data.TE = [3.92, 8.288, 12.656, 17.024, 21.392];
...
pad_size = [12, 12, 12];
Data.VoxelSize = double([Data.VoxelSize(:).']);  % force to 1xN numeric row

% Call without the transpose on VoxelSize
Data.QSM = QSM_iLSQR( ...
    Data.local_field, ...
    Data.mask_brain_new, ...
...
if ~(sum(RunOptions.EvenSizePadding) == 0)
    input_field = {'x_para', 'x_dia', 'x_tot','qsm_map', 'QSM','Mask','R2s','R2p','UnwrappedPhase','local_field','local_field_hz','mask_brain_new','vesselMask_para','vesselMask_dia', 'MGRE_Mag_Tukey', 'MGRE_Phs_Tukey'};
    for i = 1:length(input_field)
...
cvdproc_directory = 'E:/Codes/cvdproc';
addpath(genpath(fullfile(cvdproc_directory, 'cvdproc', 'utils', 'matlab', 'QSM_pipeline_part1_func')))
