...
| FSL truenet | 'truenet' | 3D FLAIR / T1w / 3D FLAIR + T1w | [FSL truenet](https://github.com/v-sundaresan/truenet). The advantages of this method include the provision of multiple models, suitable for single-channel FLAIR and T1w or dual-channel FLAIR + T1w, especially FLAIR + T1w will get very good results, but the preprocessing time is relatively long (because it involves steps such as FAST segmentation). |

!!! note "truenet"
    We omit the preprocessing steps because we do not involve fine-tuning the model. This can significantly speed up the processing pipeline while achieving similar results.
...
- 'JHU': this is achieved by non-linearly registering the JHU atlas in standard space to individual space [@biesbroek2025signature]. However, it should be noted that common PWMH may not fall within any ROI (in other words, it may be more suitable for subjects with more severe WMH).
...
If 'shape_features' is set to True, shape features of PWMH and DWMH will be calculated. We refer to the SMART-MR studies (e.g., Keller et al. [@keller2023white], Ghaznawi et al. [@ghaznawi2019association]) and Han et al. [@han2021association].
